...
COUNTRIES = [
    ('cl', 'Chile', 'https://www.laborum.cl'),
    ('pe', 'Perú', 'https://www.laborum.pe'),
    ('ar', 'Argentina', 'https://www.laborum.com.ar'),
]
...
def _parse_page(page_html, base_url, country_tld, country_name):
    ...
    # Laborum selectors - try multiple patterns
    cards = (
        tree.cssselect('.aviso-box') or
        tree.cssselect('.box-aviso') or
        tree.cssselect('[class*="JobCard"]') or
        tree.cssselect('[class*="job-item"]') or
        tree.cssselect('[class*="oferta"]') or
    ...
    for card in cards:
        try:
            ...
            title_el = (card.cssselect('h2 a') or card.cssselect('h2') or
                       card.cssselect('.titulo-aviso') or card.cssselect('[class*="title"]') or
                       card.cssselect('a'))
            ...
            # Link
            link_el = (card.cssselect('h2 a') or card.cssselect('a[href*="/trabajo/"]') or
                      card.cssselect('a[href*="/empleo/"]') or card.cssselect('a'))
            link = ''
            if link_el:
            ...
            jobs.append({
                'id': make_id(f'laborum-{country_tld}', link),
                'source': f'laborum-{country_tld}',
                'sourceUrl': link,
                'title': title,
                'company': company,
                'location': location,
...
def scrape_laborum(max_jobs=200):
    ...
    for tld, name, base_url in COUNTRIES:
        ...
        log.info(f'Scraping Laborum {name} ({tld})... ({len(all_jobs)}/{max_jobs})')
        country_jobs = []
        ...
        for qi, query in enumerate(QUERIES):
            ...
            for page in range(1, 3):
                ...
                try:
                    url = f"{base_url}/trabajo/?q={query.replace(' ', '+')}&p={page}"
                    ...
                    if 'captcha' in resp.text.lower() or 'challenge' in resp.url:
                        log.warning(f'Laborum {tld}: blocked at query {qi}')
                        break
                    ...
                    log.warning(f'Laborum {tld} "{query}" p{page}: {e}')
                    break

            time.sleep(2)

        log.info(f'Laborum {name}: {len(country_jobs)} jobs')
...
if __name__ == '__main__':
    ...
    jobs = scrape_laborum()
    print(f'\nTotal: {len(jobs)} jobs from Laborum')
